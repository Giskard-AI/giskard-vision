...
@pytest.fixture()
def dataset_300w():
    return DataLoader300W(dir_path=Path("examples") / "landmark_detection" / "300W" / "sample")


@pytest.fixture()
def dataset_300w_batched():
    return DataLoader300W(dir_path=Path("examples") / "landmark_detection" / "300W" / "sample", batch_size=5)


@pytest.fixture()
def dataset_ffhq():
    return DataLoaderFFHQ(dir_path=Path("examples") / "landmark_detection" / "ffhq")
