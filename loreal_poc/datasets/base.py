...
@dataclass(frozen=True)
class FacialParts:
    all: FacialPart = FacialPart(np.arange(0, 68))
...
class Base(ABC):
    def __init__(self,
        ...
        self.image_paths = self._get_all_paths_based_on_suffix(images_dir_path, image_suffix)
        self.marks_paths = self._get_all_paths_based_on_suffix(landmarks_dir_path, marks_suffix)
    ...
    def _get_all_paths_based_on_suffix(cls, dir_path: Union[str, Path], suffix: str):
        ...
        if len(all_paths_with_suffix) == 0:
            raise ValueError(f"{cls.__class__.__name__}: Landmarks with suffix {suffix}"
    ...
    def all_marks_for(self, part: FacialPart, exclude=False):
        idx = ~np.isin(ALL, part) if not exclude else np.isin(ALL, part)
