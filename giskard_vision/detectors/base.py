...
class ScanResult:
    ...
    Attributes:
        ...
        group (IssueGroup): Group name for the issue
...
class DetectorVisionBase(Detector):
    ...
    def get_issues(
        self, model: Any, dataset: Any, results: List[ScanResult], issue_levels: Tuple[IssueLevel]
    ) -> Sequence[Issue]:
        """
        Returns a list of giskard Issue from results output by get_results
        ...
        Args:
            ...
            issue_levels (Tuple[IssueLevel]): issue levels that will be displayed (IssueLevel.MAJOR, IssueLevel.MEDIUM, IssueLevel.MINOR)
