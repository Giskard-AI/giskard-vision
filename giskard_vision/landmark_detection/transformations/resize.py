...
def resize_image_with_marks(
    img: np.ndarray, marks: LandmarkTypes.label | None, scales: Union[Tuple[float, float], float], absolute_scales: bool
) -> np.ndarray:
...
def resize_marks(marks: LandmarkTypes.label, scales: Union[Tuple[float, float], float]) -> np.ndarray:
